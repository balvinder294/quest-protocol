...
export const TetrisGame: React.FC = () => {
  ...
  const moveDown = useCallback(() => {
    ...
    if (isValidMove(currentPiece, currentPiece.x, currentPiece.y + 1, grid)) {
      setCurrentPiece((p: any) => ({ ...p, y: p.y + 1 }));
    } else {
      lockPiece();
    }
  ...
  const handleAction = (type: 'LEFT' | 'RIGHT' | 'DOWN' | 'ROTATE') => {
    ...
    if (type === 'LEFT') {
      if (isValidMove(currentPiece, currentPiece.x - 1, currentPiece.y, grid)) {
        setCurrentPiece((p: any) => ({ ...p, x: p.x - 1 }));
      }
    } else if (type === 'RIGHT') {
      if (isValidMove(currentPiece, currentPiece.x + 1, currentPiece.y, grid)) {
        setCurrentPiece((p: any) => ({ ...p, x: p.x + 1 }));
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-8">
        <RouterLink to="/games" className="flex items-center text-slate-400 hover:text-white transition group">
